...
def create_item(item: schemas.ItemCreate, db: Session = Depends(get_db)):
    ...
    return crud.create_item(db, item, user=None)
...
def update_item(item_id: int, item: schemas.ItemCreate, db: Session = Depends(get_db)):
    db_item = crud.update_item(db, item_id, item, user=None)
    if not db_item:
        raise HTTPException(status_code=404, detail="Item not found")
    return db_item
...
def delete_item(item_id: int, db: Session = Depends(get_db)):
    success = crud.delete_item(db, item_id, user=None)
    if not success:
        raise HTTPException(status_code=404, detail="Item not found")
    return {"ok": True}
...
def get_audit_log(db: Session = Depends(get_db)):
    return db.query(model.AuditLog).order_by(model.AuditLog.timestamp.desc()).all()